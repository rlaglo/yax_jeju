...
from collections import deque # 김해창
...
class Yolov8InfoExtractor(Node):
    ...
    def yolov8_detections_callback(self, detection_msg: DetectionArray):
        ...
        self.point_history.append(target_points) # 김해창
        if self.show_image: 
                    # 컬러 점을 찍기 위해 흑백 ROI 이미지를 BGR 컬러 이미지로 변환
                    vis_image = cv2.cvtColor(roi_image, cv2.COLOR_GRAY2BGR)

                    # 과거의 점들 그리기 (오래된 점일수록 작고 흐리게)
                    for i, frame_points in enumerate(self.point_history):
                        # 최근 데이터일수록 색상을 진하게 (초록색 계열)
                        intensity = int(255 * (i + 1) / len(self.point_history))
                        color = (0, intensity, 0) 
                        
                        for pt in frame_points:
                            cv2.circle(vis_image, (pt.target_x, pt.target_y), 2, color, -1)

                    # 현재 프레임의 최신 점들은 빨간색으로 크게 표시
                    for pt in target_points:
                        cv2.circle(vis_image, (pt.target_x, pt.target_y), 5, (0, 0, 255), -1)

                    cv2.imshow(f'roi_img_{self.cam_num}', vis_image)
                    cv2.waitKey(1)
